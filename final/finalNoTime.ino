    ...
    */
    ...
    para preto LDR1 < 2970
Para branco LDR2 < 3430
LDR0 < 3640

void ColorsLDR() {
    for (int x = 0; x < QTD_LDR; x++) {
        if (x == 0) { // LDR0
            if (mediaMovelLDR[x] > 3640) {
                ColorLDR[x] = 1; // Preto
            } else {
                ColorLDR[x] = 0; // Branco
            }
        } else if (x == 1) { // LDR1
            if (mediaMovelLDR[x] > 2970) {
                ColorLDR[x] = 1; // Preto
            } else {
                ColorLDR[x] = 0; // Branco
            }
        } else if (x == 2) { // LDR2
            if (mediaMovelLDR[x] > 3430) {
                ColorLDR[x] = 1; // Preto
            } else {
                ColorLDR[x] = 0; // Branco
            }
        ...
        Serial.print(x);
        Serial.print(": ");
        Serial.print(mediaMovelLDR[x]);
        Serial.print(" - Cor: ");
        Serial.println(ColorLDR[x] == 0 ? "Preto" : "Branco");
    }
}
...
void PegarArvoreG(){
    ...
    acionarServo(M, 45);
    delay(3000);
    //acionarServo(G, 150); // Aciona o Servo E na posição inicial
    servos[4].write(90);
    delay(3000);
